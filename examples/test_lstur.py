...
model_dir = os.path.join(data_path, "lstur")
...
def dist_eval(args):
    ...
    with open(os.path.join(data_path, 'results/lstur-valid-prediction.{}.txt'.format(args.fsplit)), 'w') as f:
...
def test(args):
    iterator = MINDIterator
    model = LSTURModel(hparams, iterator, seed=seed, test_mode=True)
    model.model.load_weights(os.path.join(model_dir, "ckpt_ep{}".format(args.ep)))
    ...
    with open(os.path.join(data_path, 'results/lstur-test-prediction.{}.txt'.format(args.fsplit)), 'w') as f:
